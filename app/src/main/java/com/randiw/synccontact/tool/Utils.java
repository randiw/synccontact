...
//    }
    ...
    public static String parsePhoneNumber(String string){
    	...
    	if(number.contains(TEL)){
    		index = number.indexOf(TEL);
    		number = number.substring(index);
    	}    	
    	if(number.contains(SMS)){
    		index = number.indexOf(SMS);
    		number = number.substring(index);
    	} 
    	if(number.contains(PLUS)){
    		index = number.indexOf(PLUS);
    		number = number.substring(index);			
    	}
    	if(number.contains(SPACE)){
    		number = number.split(SPACE)[1];
